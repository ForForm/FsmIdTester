...
namespace BarkodesDeviceVerison
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            cbManufacturer.DataSource = Enum.GetValues(typeof(BarkodesDeviceVerison.ComMng.ManufacturerCode));
        }
                    ...
                    if (cbDevices.Items.Count > 0)
                    {
                        ...
                        for (int i = 0; i < CnvIndex; i++)
                        {
                            if (cbDevices.Text == nIP[i, 0])
                                if (nIP[i, 1] == "T") { lblCon.Text = "OldConverter"; Cnv = ComMng.Converter.Tac; }
                                else { lblCon.Text = "NewConverter"; Cnv = ComMng.Converter.NewCon; }
                        }
                    }
                    else
                    {
        ...
        private void btnGetVersion_Click(object sender, EventArgs e)
        {
            ...
            {
                BarkodesDeviceVerison.ComMng.ManufacturerCode code; 
                Result = cm.GetDeviceVersionFsm(txtIP.Text, Convert.ToInt32(txtTcpPort.Text), Convert.ToInt32(txtAdr.Text), out version, out code, 500, Cnv);
                if (Result == ComMng.ReturnValues.Succesfull)
                {
                    if (code != 0)
                    {
                        cbManufacturer.SelectedItem = code;
                        cbManufacturer.Text = code.ToString();
                    }
                    else cbManufacturer.Text = "";
                    pBar.Value = 100;                 
                }
                LabelStatus.Text = Result.ToString();
            }
        ...
        private void btnSerVersion_Click(object sender, EventArgs e)
        {
            ...
            else if (cmbDeviceType.Text == "Fsm 1453")
            {
                ...
                byte version = (byte)cmbVersion.SelectedIndex;
                BarkodesDeviceVerison.ComMng.ManufacturerCode code = (ComMng.ManufacturerCode)cbManufacturer.SelectedItem;
                Result = cm.SetDeviceVersionFsm(txtIP.Text, Convert.ToInt32(txtTcpPort.Text), Convert.ToInt32(txtAdr.Text), version, code, 2000, Cnv);
        ...
        private void btnSerVersionUdp_Click(object sender, EventArgs e)
        {
            ...
            else LabelStatus.Text = "Fsm için Udp seçeğini mevcut değil";
        }
