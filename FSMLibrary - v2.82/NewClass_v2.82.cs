...
namespace FSM_Authorization
{
    ...
    public class ConnectionManager
    {
        ...
        private ReturnValues GetLicence()
        {
            ReturnValues Result = ReturnValues.Failed;

            try
            {
                //BinaryReader ReadStream = new BinaryReader(new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite));
                string fileDir = AppDomain.CurrentDomain.BaseDirectory;//System.IO.Directory.GetCurrentDirectory();
                string[] filePaths = System.IO.Directory.GetFiles(fileDir, "*.lic");
                if (filePaths.Length <= 0) return ReturnValues.Failed; //ReturnValues.LicFileNoxExist;

                for (int y = 0; y < filePaths.Length; y++)
                {
                    Result = ReturnValues.Failed; //ReturnValues.LicenseError;
                    string filePath = filePaths[y];
                    System.IO.StreamReader sr = new System.IO.StreamReader(filePath, Encoding.ASCII);

                    string str = "";

                    string[] eLicenseCode = new string[6];
                    ...
                    try
                    {
                        for (int i = 0; i < eLicenseCode.Length; i++)
                        {
                            str = sr.ReadLine();
                            string[] lic = str.Split(':');
                            eLicenseCode[i] = lic[1];
                        }
                    }
                    catch
                    {
                        sr.Close();
                        return ReturnValues.Failed; //ReturnValues.CorruptedLicFile;
                    ...
                    string[] dLicenseCode = new string[6];
                    for (int i = 0; i < dLicenseCode.Length; i++)
                    {
                        dLicenseCode[i] = SMP.DecryptString(eLicenseCode[i]);
                    }
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        if (i > 0)
                        {
                            if (dLicenseCode[i].Substring(2) != dLicenseCode[i - 1].Substring(2))
                                return ReturnValues.Failed;
                        }

                    }

                    byte[] xManufacturerCode = new byte[9];
                    _ManufacturerCode = new byte[8];
                    xManufacturerCode = Conversion.HexToByte(dLicenseCode[0]);
                    Array.Copy(xManufacturerCode, 1, _ManufacturerCode, 0, 8);
            ...
            {
                return ReturnValues.Failed; //ReturnValues.LicenseError;
            }
        }
        ...
        */
        ...
        public enum Commands
        {
            ...
            Cmd_WinterTime = 176,
            Cmd_RequestAlarm = 178,
            Cmd_Elevator = 180,
            Cmd_ElevatorConfig = 182,
            Cmd_ElevartorGates = 184,

            SetTeminalData = 186,
            GetTeminalData = 188,

            Cmd_ClkCalibration = 254,
        }
        ...
        public enum Events_v4
        {
            ...
            UpdateYearTime = 5,
            ErrorRWIndexLog = 6,
            ErrorRWIndexPerson = 7,
            ErrorRWIndexBlack = 8,
            ErrorRWIndexSpare = 9,
            ErrorRWIndexErsPerson = 10,
            AccessWithButton = 11,
            ErrRWStaffGroup = 12,
            ChangeWinterTime = 13,
            SaveDateTime = 14,
            ChangeDefineMode = 15,
            ChangeNormalMode = 16,
            ChangeServiceMode = 17,
            ChangeOfflineMode = 18,
            ChangeOnlineInfoMode = 19,
            ChangeOnlineAuthorMode = 20,
            ChangeDeviceVersion = 21,
            DeleteAllPerson = 22,
            DeleteBlackList = 23,
            DeleteAllAccessLog = 24,
            DeleteAllSpareLog = 25,
            DeleteAllEventLog = 26,
            DeviceRestart = 27,
            SaveCardKeys = 28,
            SaveDeviceInfo = 29,
            SaveConfiguration = 30,
            AutoDefineMode = 31,
            AutoNormalMode = 32,
            AutoServiceMode = 33,
            AutoOfflineMode = 34,
            AutoOnlineInfoMode = 35,
            AutoOnlineAuthorMode = 36,
            StartingDevice = 37,
            AutoServisForAddUser = 38,
            AutoServisForAddBlack = 39,
            AutoServisForGetAccess = 40,
            AutoServisForGetSpare = 41,
            AutoServisForGetEvent = 42,
            AutoServisForGetUser = 43,
            AutoServisForGetBlack = 44,
            NoEvent = 255,
        }
        ...
        public ReturnValues ListenOnlineRequest(out UInt64 ID, out int Address, out int OfflineLogCount, out EmergencySts Emergency, out DoorStatus Door, out CardStatus card, TcpClient client, int msTimeOut, Converter cnv, out byte[] temp)
        {
            ID = 0;
            Address = 0;
            temp = null;
            OfflineLogCount = 0;
            card = CardStatus.NoCard;
            Door = DoorStatus.CloseDoor;
            Emergency = EmergencySts.No;
                    ...
                    if (rv == ReturnValues.Successful)
                    {
                        if (temp != null)
                        {
                            ...
                            if (crc == temp[temp[0] - 1])
                            {
                                ...
                                else if ((temp[4] == (byte)Commands.RequestPending) && ((temp[5] == 0) || (temp[5] == 1)))
                                {
                                    ID = temp[6];
                                    ID = (ID << 8) | (temp[7]);
                                    ID = (ID << 8) | (temp[8]);
        ...
        public ReturnValues CalibreTest(string TargetIP, int TargetPort, int TargetAddress, TcpClient client, out string cal, int msTimeOut, Converter cnv)
        {
            cal = "";
            try
            {
                if (client.Connected)
                {
                    ...
                    if (TibboDataMode(client, msTimeOut, cnv) == ReturnValues.Successful)
                    {
                        ...
                        ReturnValues Result1 = GetDataStream_Test(out packet, out length, client, msTimeOut);
                        if (Result1 == ReturnValues.Successful)
                        {
                            byte[] message = new byte[42];
                            Array.Copy(packet, 6, message, 0, message.Length);
                            cal = System.Text.ASCIIEncoding.ASCII.GetString(message);
                            return ReturnValues.Successful;
                        }
        ...
        public ReturnValues SetCalibration(string TargetIP, int TargetPort, int TargetAddress, TcpClient client, byte cal, int msTimeOut, Converter cnv)
        {
            int b = 0;
            try
            {
                ...
                if (TibboDataMode(client, msTimeOut, cnv) == ReturnValues.Successful)
                {
                    ...
                    stream[0] = (byte)stream.Length;     	/*Packet Length*/
                    stream[1] = (byte)DataStruct.PCToReader;    /* Prefix  */
                    stream[2] = 0xff;            /* Device Address  */
                    stream[3] = cal;      					/* Length  */
                    stream[4] = (byte)Commands.Cmd_ClkCalibration;     /* Command */
                    stream[5] = 0;     						/* SubCommand */
                    ...
                    if (Result1 == ReturnValues.Successful)
                    {
                        ...
                        if (Result1 == ReturnValues.Successful)
                        {
                                if ((packet[1] == (byte)DataStruct.ReaderTOPC))
                                    if (packet[4] == (byte)(stream[4]) + 1 && packet[5] == (byte)(stream[4]) + 1)
                                    {
                                        return ReturnValues.Successful;
                                    }
                                    else { return ReturnValues.InvalidResponse; }
        ...
        public ReturnValues SaveElevatorConfig(string TargetIP, int TargetPort, int Address, Contact relayState, int msTimeOut, Converter cnv)
        {
            //int a = 0;
            //int b = 0;
            try
            {
                ...
                if (TibboDataMode(client, msTimeOut, cnv) == ReturnValues.Successful)
                {
                    stream = new byte[9];                           /*PersonalDataSize+DefaultLength*/
                    stream[0] = (byte)stream.Length;     				/*Packet Length*/
                    stream[1] = (byte)DataStruct.PCToReader;    	        /* Prefix  */
                    stream[2] = 0;            /* Device Address  */
                    stream[3] = (byte)(stream.Length - 4);      	    /* Length  */
                    stream[4] = (byte)Commands.Cmd_ElevatorConfig;               /* Command */
                    stream[5] = 0;     				                    /* SubCommand */

                    stream[6] = (byte)Address;
                    stream[7] = (byte)relayState;
        ...
        public ReturnValues RecordAPersonNew(string TargetIP, int TargetPort, int TargetAddress, UInt64 PersonId, byte Shift,  byte[] Gates, int msTimeOut, Converter cnv)
        {
            int b = 0;
            try
            {
                ...
                if (TibboDataMode(client, msTimeOut, cnv) == ReturnValues.Successful)
                {
                    stream = new byte[7+7+1+16];                          /*PersonalDataSize+DefaultLength*/
                    stream[0] = (byte)stream.Length;     				/*Packet Length*/
                    stream[1] = (byte)DataStruct.PCToReader;    	        /* Prefix  */
                    stream[2] = (byte)TargetAddress;            /* Device Address  */
                    stream[3] = (byte)(stream.Length - 4);      	    /* Length  */
                    ...
                    stream[13] = Shift;
                    if (Gates.Length > 0)
                        Array.Copy(Gates, 0, stream, 14, Gates.Length);
        ...
        public ReturnValues GetEventLog_old(string TargetIP, int TargetPort, int TargetAddress, long Index, out byte[] eventLog, out Events_v4 code, out DateTime EventTime, int msTimeOut, Converter cnv)
        {
            int b = 0; eventLog = null;
            try
            {
                StartAgain:
                code = Events_v4.NoEvent;
                EventTime = new DateTime();
                ...
                if (TibboDataMode(client, msTimeOut, cnv) == ReturnValues.Successful)
                {
                    ...
                    if (Result1 == ReturnValues.Successful)
                    {
                        ...
                        Array.Copy(Response, 6, eventLog, 0, eventLog.Length);
        ...
        public ReturnValues GetPersonCardID(string TargetIP, int TargetPort, int TargetAddress, int Index, out UInt64 PerID, out PersonState State, out byte Shift, out byte[] Gates, int msTimeOut, Converter cnv)
        {
            int b = 0;
            State = 0; Shift = 0; Gates = null;
            PerID = 0;
            try
            {
                StartAgain:
                                ...
                                if (crc == Response[length - 1])//true
                                    if ((Response[1] == (byte)DataStruct.ReaderTOPC) & (Response[2] == (byte)TargetAddress))
                                        if (Response[4] == (byte)(stream[4]) + 1)
                                        {
                                            ...
                                            Gates = new byte[16];
                                            Array.Copy(Response, 15, Gates, 0, 16);
                                ...
                                if (crc == Response[length - 1])//true
                                    if ((Response[1] == (byte)DataStruct.ReaderTOPC) & (Response[2] == (byte)TargetAddress))
                                        if (Response[4] == (byte)(stream[4]) + 1)
                                        {
                                            ...
                                            PerID = (UInt32)(Response[13]);
                                            PerID = (UInt32)((PerID << 8) | (Response[12]));
                                            PerID = (UInt32)((PerID << 8) | (Response[11]));
                                            PerID = (UInt32)((PerID << 8) | (Response[10]));
                                            PerID = (UInt32)((PerID << 8) | (Response[9] ));
                                            PerID = (UInt32)((PerID << 8) | (Response[8] ));
                                            PerID = (UInt32)((PerID << 8) | (Response[7] ));
                                            PerID = (UInt32)((PerID << 8) | (Response[6] ));
                                            client.Close();
                                            return ReturnValues.Successful;
                                        }
                                        else { client.Close(); return ReturnValues.InvalidResponse; }
        ...
        public ReturnValues ListenOnlineRequest(TcpClient client, int Addrss, out UInt64 ID, out int Address, AccessType Accss, BuzzerState Buzzer, byte[] Gates, int RelayTime, out int OfflineLogCount, int msTimeOut, Converter cnv, out byte[] temp)
        {
            int b = 0;
            ID = 0;
            Address = 0;
                            ...
                            if (temp != null)
                            {
                                ...
                                if (crc == temp[temp[0] - 1])
                                {
                                    ...
                                    if ((temp[4] == (byte)Commands.RequestPending) & (temp[5] == (byte)Commands.RequestID) && length > 15)
                                    {
                                        ...
                                        Result = SendAccess(client, Addrss, Accss, RelayTime, Buzzer, Gates, msTimeOut, cnv);
                                        if (Result == ConnectionManager.ReturnValues.Successful)
                                        {
                                            return ReturnValues.Successful;
                                        }
        ...
        public ReturnValues SendAccess(string TargetIP, int TargetPort, int TargetAddress, AccessType Access, int RelayTime, BuzzerState Buzzer, byte[] Gates, int msTimeOut, Converter cnv)
        {
            int b = 0;
            ...
            try
            {
                ...
                if (TibboDataMode(client, msTimeOut, cnv) == ReturnValues.Successful)
                {
                    ...
                    if (Gates.Length > 0)
                        Array.Copy(Gates, 0, stream, 8, Gates.Length);
        ...
        public ReturnValues SendAccess(TcpClient client, int TargetAddress, AccessType Access, int RelayTime, BuzzerState Buzzer, byte[] Gates, int msTimeOut, Converter cnv)
        {
            int b = 0;
            try
            {
        ...
        public ReturnValues SendTerminalData(string TargetIP, int TargetPort, int TargetAddress, byte[] data, int msTimeOut, Converter cnv)
        {
            int b = 0;
            ...
            try
            {
                ...
                if (TibboDataMode(client, msTimeOut, cnv) == ReturnValues.Successful)
                {
                    stream = new byte[7 + data.Length];
                    stream[0] = (byte)stream.Length;     				/*Packet Length*/
                    stream[1] = (byte)DataStruct.PCToReader;    	        /* Prefix  */
                    stream[2] = (byte)TargetAddress;            /* Device Address  */
                    stream[3] = (byte)data.Length;      		/* Length  */
                    stream[4] = (byte)Commands.SetTeminalData;
                    stream[5] = 0;     				                   /* SubCommand */

                    Array.Copy(data, 0, stream, 6, data.Length);
        ...
        public ReturnValues GetTerminalData(string TargetIP, int TargetPort, int TargetAddress, out byte[] data, int msTimeOut, Converter cnv)
        {
            int b = 0;
            data = null;
            ...
            try
            {
                ...
                if (TibboDataMode(client, msTimeOut, cnv) == ReturnValues.Successful)
                {
                    ...
                    stream[1] = (byte)DataStruct.PCToReader;    	        /* Prefix  */
                    stream[2] = (byte)TargetAddress;            /* Device Address  */
                    stream[3] = (byte)0;      		/* Length  */
                    stream[4] = (byte)Commands.GetTeminalData;
                    stream[5] = 0;     				                   /* SubCommand */
                            ...
                            if (crc == Response[length - 1])
                                if ((Response[1] == (byte)DataStruct.ReaderTOPC) & (Response[2] == (byte)TargetAddress))
                                    if (Response[4] == (byte)(stream[4]) + 1)
                                    {
                                        ...
                                        Array.Copy(Response, 6, data, 0, Response[3]);
                                        client.Close();
                                        return ReturnValues.Successful;
                                    }
                                    else { client.Close(); return ReturnValues.InvalidResponse; }
        ...
        public ReturnValues CardResponse(TcpClient client, int TargetAddress, CardStatus card, int msTimeOut, Converter cnv)
        {
            int b = 0;
            try
            {
                ...
                stream[2] = (byte)TargetAddress;            /* Device Address  	*/
                stream[3] = (byte)(stream.Length - 4);      /* Length  			*/
                stream[4] = (byte)Commands.RequestPending;
                stream[5] = (byte)card;     				                   /* SubCommand */
        ...
        public ReturnValues Access(TcpClient client, int TargetAddress, AccessType Access, int RelayTime, BuzzerState Buzzer, byte[] Gates, int msTimeOut, Converter cnv)
        {
            int b = 0;
            try
            {
                byte[] stream;
                stream = new byte[7+16+1+1];
                stream[0] = (byte)stream.Length;     		/* Packet Length    */
                stream[1] = (byte)DataStruct.PCToReader;    /* Prefix  			*/
                stream[2] = (byte)TargetAddress;            /* Device Address  	*/
                stream[3] = (byte)(stream.Length - 4);      /* Length  			*/
                ...
                if (len > 16) len = 16;
                Array.Copy(Gates, 0, stream, 8, len);
                stream[stream.Length - 1] = 0;
                for (int i = 0; i < stream.Length - 1; i++)
                {
                    stream[stream.Length - 1] ^= stream[i];
        ...
        public ReturnValues ChangeConfigParameters(string TargetIP, int TargetPort, int TargetAddress, byte[] cfg, int msTimeOut, Converter cnv)
        {
            ...
            StartAgain:
                ...
                {
                    int CfgLength = 17;
                    stream = new byte[28];//[CfgLength + 7 + 20];
                    stream[0] = (byte)stream.Length;     				/*Packet Length*/
                    stream[1] = (byte)DataStruct.PCToReader;    	        /* Prefix  */
                    stream[2] = (byte)TargetAddress;            /* Device Address  */
                    stream[3] = (byte)(stream.Length - 4);      		/* Length  */
                                    ...
                                    if (Response[4] == (byte)(stream[4]) + 1)
                                    {
                                        try
                                        {
                                            ...
                                            if (Configuration.Length == 0)
                                                return (ReturnValues)Response[4];
                            ...
                            if (crc == Response[length - 1]/*true*/)
                                if ((Response[1] == (byte)DataStruct.ReaderTOPC) & (Response[2] == (byte)TargetAddress))
                                    if (Response[4] == (byte)(stream[4]) + 1)
                                    {
                                        ...
                                        Application = ASCIIEncoding.ASCII.GetString(Response, 10 + i, 7);
                                        PcbVersion = ASCIIEncoding.ASCII.GetString(Response, 17 + i, 3);
                                        PcbVersion = PcbVersion.Insert(2, "R");
                                        FirmwareVersion = ASCIIEncoding.ASCII.GetString(Response, 20 + i, 3);
